...
class RevokeRoleCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addArgument('role', InputArgument::REQUIRED, 'Role to revoke (' . implode(', ', Roles::values()) . ')');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$this->userRoleManager->hasRole($user->id, $role)) {
            $io->note(sprintf('User "%s" does not have role "%s".', $email, $role->value));
            return Command::SUCCESS;
        }

        $this->userRoleManager->removeRole($user->id, $role);
        $io->success(sprintf('Role "%s" revoked from user "%s".', $role->value, $email));
